...
from custom_components.eon_next.api_client.intelligent_device import IntelligentDevice
      ...
      if gas_tariff is not None:
        for meter in point["meters"]:
          ...
          entity_ids_to_migrate.append({
            "old": f"eon_next_gas_{serial_number}_{mprn}_previous_accumulative_consumption",
            "new": f"eon_next_gas_{serial_number}_{mprn}_previous_accumulative_consumption_m3"
          })
          ...
          if (CONFIG_MAIN_SUPPORTS_LIVE_CONSUMPTION in config and config[CONFIG_MAIN_SUPPORTS_LIVE_CONSUMPTION] == True):
            ...
            if meter["device_id"] is not None and meter["device_id"] != "":
              ...
              entity_ids_to_migrate.append({
                "old": f"eon_next_gas_{serial_number}_{mprn}_current_accumulative_consumption",
                "new": f"eon_next_gas_{serial_number}_{mprn}_current_accumulative_consumption_kwh"
              })
