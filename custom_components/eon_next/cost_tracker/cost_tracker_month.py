...
class OctopusEnergyCostTrackerMonthSensor(RestoreSensor):
  ...
  def unique_id(self):
    ...
    base_name = f"eon_next_cost_tracker_{self._config[CONFIG_COST_TRACKER_NAME]}_month"
  ...
  def name(self):
    """Name of the sensor."""
    base_name = f"EON Next Cost Tracker {self._config[CONFIG_COST_TRACKER_NAME]} Month"
    if self._peak_type is not None:
